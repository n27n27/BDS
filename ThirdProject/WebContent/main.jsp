...
<body>
	<div id=wrap>
		<div id=header>
			<div id="write27" ><a href='/main.jsp'><img src='./img/home.png' width=20px>BDS</a></div>
			<div class="write27">
				<c:set var="hid" value="<%= (String)session.getAttribute(\"id\") %>" />
				<c:choose>
					<c:when test = "${hid == null }">
						<div class=menu1>Login</div>							
					</c:when>
					<c:otherwise>
						<div class=menu1>${hid} 님</div>
						<div class=menu1>LogOut</div>
					</c:otherwise>
				</c:choose>		
				<div class=menu1>문의</div>
				<div class=menu1>샘플</div>
			</div>
	 	</div><br><br>
	 	<hr>
